...
const CITATIONS = [
	{
		...
		insight:
			"Your word choices literally reveal who you are emotionally and personality-wise.",
	},
	{
		...
		insight:
			"When you really 'click' with someone, your language syncs up—that's a real thing.",
	},
	{
		...
		insight:
			"The way your texts align with others predicts whether you'll actually get along and work well together.",
	},
	{
		...
		insight:
			"This is where the whole Giver–Taker–Matcher framework comes from—it's legit social science.",
	},
	{
		...
		insight:
			"Even tiny linguistic similarities predict whether relationships will actually last.",
	},
];
...
const TOKEN_PACKAGES: TokenPackage[] = [
	...
	{
		priceId: "price_1SL21MCvKngy5LHXVvlL6oMU",
		tokens: 15,
		price: 9.99,
		savings: 5,
	},
	{
		priceId: "price_1SL21MCvKngy5LHXvy8LtMuA",
		tokens: 40,
		price: 19.99,
		savings: 20,
	},
...
function App() {
	...
	useEffect(() => {
		if (isAnalyzing) {
			const randomSlogan =
				PLAYFUL_SLOGANS[Math.floor(Math.random() * PLAYFUL_SLOGANS.length)];
	...
	const parsePersonSection = (section: string): PersonReport | null => {
		...
		const socialVibeMatch = section.match(
			/🪶\s+Social Vibe[:\s]*([0-9.]+)\s*\/\s*10\s*[—-]\s*\[([^\]]+)\]/i
		);
		...
		const reciprocityMatch = section.match(
			/\*\*Your Reciprocity Style:\*\*\s*([0-9.]+)\s*\/\s*10|\*\*Reciprocity Style:\*\*\s*([0-9.]+)\s*\/\s*10/i
		);
		const reciprocityStyle = reciprocityMatch
			? parseFloat(reciprocityMatch[1] || reciprocityMatch[2])
			: 5;
		const reciprocityCategory = getReciprocityCategory(reciprocityStyle);

		// Extract Social Presence: X / 10
		const socialPresenceMatch = section.match(
			/\*\*Your Social Presence:\*\*\s*([0-9.]+)\s*\/\s*10|\*\*Social Presence:\*\*\s*([0-9.]+)\s*\/\s*10/i
		);
		const socialPresence = socialPresenceMatch
			? parseFloat(socialPresenceMatch[1] || socialPresenceMatch[2])
			: 5;
		...
		const patternMatch = section.match(
			/\*\*Your Communication Pattern:\*\*\s*([^\n]+(?:\n(?!\*\*Your|\*\*[A-Z])[^\n]*)*?)|\*\*Communication Pattern:\*\*\s*([^\n]+(?:\n(?!\*\*[A-Z])[^\n]*)*?)/i
		);
		const communicationPattern = patternMatch
			? (patternMatch[1] || patternMatch[2] || "").trim()
			: "";
...
CHAT TO ANALYZE:
 ${chat}`;
			...
			const groupScoreMatch = fullReport.match(
				/🌐\s+Group Social Vibe\s*\nScore[:\s]*([0-9.]+)\s*\/\s*10\s*[—-]\s*([^\n]+)/i
			);
			const groupScore = groupScoreMatch ? parseFloat(groupScoreMatch[1]) : 5.5;
			const groupSummary = groupScoreMatch
				? groupScoreMatch[2].trim()
				: "A mix of energies and dynamics";
	...
	const generateShareCaption = () => {
		if (!groupReport || peopleReports.length === 0)
			return "Check out my social vibe report! 👇";
		...
		try {
			...
			if (navigator.share && navigator.canShare) {
				try {
					...
					const file = new File([blob], `vibe-report-${Date.now()}.png`, {
						type: "image/png",
					});
	...
	const handleBuyTokens = async (tokenPackage: TokenPackage) => {
		...
		try {
			...
			const response = await blink.data.fetch({
				...
				body: `line_items[0][price]=${
					tokenPackage.priceId
				}&line_items[0][quantity]=1&mode=payment&success_url=${encodeURIComponent(
					successUrl
				)}&cancel_url=${encodeURIComponent(cancelUrl)}&metadata[user_id]=${
					user.id
				}&metadata[tokens]=${
					tokenPackage.tokens
	...
	const renderPersonCard = (person: PersonReport) => (
		<div
			key={person.name}
			className="bg-white rounded-lg border border-purple-100 p-6 shadow-sm hover:shadow-md transition-shadow"
		>
			<div className="mb-4">
				...
				<div className="flex items-start gap-2">
					...
					<div>
						<p className="text-purple-900 font-bold">
							Social Vibe: {person.socialVibeScore.toFixed(1)}/10
						</p>
				...
				<div>
					<p className="text-purple-600 font-medium">
						{person.isCurrentUser
							...
							: "Reciprocity Style"}
						: {person.reciprocityStyle} / 10 ({person.reciprocityCategory})
					</p>
				</div>
				<div>
					<p className="text-purple-600 font-medium">
						{person.isCurrentUser ? "Your Social Presence" : "Social Presence"}:{" "}
						{person.socialPresence} / 10 ({person.socialPresenceCategory})
					</p>
				</div>
					...
					<div className="flex items-center gap-3">
						...
						>
							...
							{user && isLoadingCredits
								? "..."
								: `${user ? credits : guestCredits} credits`}
						</Button>
						<Button
				...
				{!report ? (
					<div className="animate-fade-in">
						<div className="text-center mb-12">
							...
							<p className="text-lg text-muted-foreground max-w-2xl mx-auto mb-6">
								Paste your group chat. The AI shows who gives, who takes, and
								who's just... there. Like having an expert psychologist in the
								room with you (but not so creepy).
							</p>
						</div>
							...
							<div className="space-y-6">
								...
								<div>
									...
									<p className="text-xs text-muted-foreground mt-2">
										Works with any chat format. The AI will automatically detect
										participants.
									</p>
								</div>
						...
						<div className="grid md:grid-cols-3 gap-4 mt-8">
							<Card className="p-6 border-purple-100 bg-white/60">
								...
								<p className="text-sm text-muted-foreground">
									See how your tone, rhythm, and presence come through in
									messages — your emotional fingerprint in text.
								</p>
							</Card>
							<Card className="p-6 border-pink-100 bg-white/60">
								...
								<p className="text-sm text-muted-foreground">
									Find out who gives, who takes, and how balanced your
							...
							<Card className="p-6 border-purple-100 bg-white/60">
								...
								<h3 className="font-semibold mb-2">
									💫 Your Social Vibe Score
								</h3>
								<p className="text-sm text-muted-foreground">
									Get one number that captures closeness, trust, and how in-sync
					...
					<div className="animate-fade-in space-y-6" ref={reportRef}>
						<div
							...
							role="img"
							aria-label={`Vibe Report for ${userName} — ${
								peopleReports.find((p) => p.isCurrentUser)
									?.reciprocityCategory || "Balanced"
							}, score ${groupReport?.score.toFixed(1) || "N/A"}/10`}
						>
							...
							{groupReport && (
								<div className="bg-gradient-to-r from-purple-600 to-pink-600 rounded-lg p-8 text-white text-center shadow-lg">
									...
									<div className="text-5xl font-bold mb-4">
										{groupReport.score.toFixed(1)} / 10
									</div>
									...
									<div className="flex justify-center gap-4 items-center h-16">
										{groupReport.score >= 7.5 && (
											<>
												<span className="text-4xl animate-emoji-pop-pulse-1">
													🎉
												</span>
												<span className="text-4xl animate-emoji-pop-pulse-2">
													✨
												</span>
												<span className="text-4xl animate-emoji-pop-pulse-3">
													🎊
												</span>
											</>
										)}
										{groupReport.score >= 5 && groupReport.score < 7.5 && (
											<>
												<span className="text-4xl animate-emoji-float-pulse-1">
													😊
												</span>
												<span className="text-4xl animate-emoji-float-pulse-2">
													💬
												</span>
												<span className="text-4xl animate-emoji-float-pulse-3">
													👫
												</span>
											</>
										)}
										{groupReport.score < 5 && (
											<>
												<span className="text-4xl animate-emoji-slow-pulse-1">
													🤔
												</span>
												<span className="text-4xl animate-emoji-slow-pulse-2">
													💭
												</span>
												<span className="text-4xl animate-emoji-slow-pulse-3">
													👤
												</span>
											</>
										)}
							...
							{peopleReports.length > 0 ? (
								...
								<Card className="p-8 text-center border-purple-200">
									<p className="text-muted-foreground">
										No individual reports were parsed.
									</p>
								</Card>
							)}
							...
							<Card className="p-6 border-purple-200">
								<Collapsible>
									...
									<CollapsibleContent className="mt-4 space-y-4">
										<p className="text-sm text-muted-foreground">
											This report isn't just vibes—it's built on actual research
											about how language reveals personality and connection.
										</p>
										...
										<p className="text-xs text-muted-foreground italic mt-4">
											Disclaimer: This report is a playful snapshot of your chat
											patterns, not a personality diagnosis. Use it for fun
											insights, not life decisions. ☕
										</p>
									</CollapsibleContent>
								</Collapsible>
							</Card>
								...
								<div className="flex items-start gap-3">
									...
									<div>
										...
										<p className="text-xs text-green-700 mt-2">
											Enter this code at checkout to test with any credit card —
							...
							<div className="border-t pt-6">
								...
								<ul className="space-y-2 text-sm text-gray-700">
									<li className="flex items-center gap-2">
										<span className="text-purple-600">✓</span> Full AI vibe
										analysis
									</li>
									<li className="flex items-center gap-2">
										<span className="text-purple-600">✓</span> Per-person
										breakdown
									</li>
									<li className="flex items-center gap-2">
										<span className="text-purple-600">✓</span> Share & export
										reports
									</li>
									<li className="flex items-center gap-2">
										<span className="text-purple-600">✓</span> No expiration on
										credits
									</li>
								</ul>
							</div>
